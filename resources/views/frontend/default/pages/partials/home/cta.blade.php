            ...
            <div class="col-lg-8 col-md-9">
                <div class="subscribe-info-wrap text-center position-relative z-2">
                    <div class="section-heading">
                        <h4 class="h5 text-danger">{{ getSetting('cta_colored_title') }}</h4>
                        <h2 class="text-light">{{ getSetting('cta_heading_title') }}</h2>
                        <p>{{ getSetting('cta_short_description') }}</p>
                    </div>
                    <div class="form-block-banner mw-60 m-auto mt-5">
                        <a href="{{ getSetting('cta_btn_link') }}"
                            class="btn btn-primary">{{ getSetting('cta_btn_title') }}</a>
                    </div>
                    <ul class="nav justify-content-center list-unstyled mt-4">
                        @php
                            $features = getSetting('cta_features') != null ? explode(',', getSetting('cta_features')) : [];
                        @endphp
