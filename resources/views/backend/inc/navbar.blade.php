                    ...
                    @if ($user->user_type == 'customer')
                        <li class="nav-item me-2">
                            <a href="{{ route('subscriptions.index') }}"
                                class="btn btn-sm {{ $user->subscription_package_id == null ? 'btn-soft-danger' : 'btn-primary' }} text-capitalize rounded-pill">
                                <i data-feather="zap" class="w-15"></i>
                                @if ($user->subscription_package_id == null)
                                    {{ localize('No active Subscription') }}
                                @else
                                    {{ $user->subscriptionPackage->title }}/{{ $user->subscriptionPackage->package_type == 'starter' ? localize('Monthly') : $user->subscriptionPackage->package_type }}
                                @endif
                            </a>
                        </li>
                    @endif
